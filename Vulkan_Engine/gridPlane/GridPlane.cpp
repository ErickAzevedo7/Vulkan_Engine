...
void GridPlane::cleanup() {
  ...
  vkDestroyPipelineLayout(VulkanCore::getDevice(), gridPlanePipelineLayout,
                          nullptr);
  vkDestroyDescriptorPool(VulkanCore::getDevice(), gridPlaneDescriptorPool,
                          nullptr);
  vkDestroyDescriptorSetLayout(VulkanCore::getDevice(),
                               gridPlaneDescriptorSetLayout, nullptr);
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroyBuffer(VulkanCore::getDevice(), gridPlaneUniformBuffers[i],
                    nullptr);
    vkFreeMemory(VulkanCore::getDevice(), gridPlaneUniformBuffersMemory[i],
                 nullptr);
    vkDestroyBuffer(VulkanCore::getDevice(), gridParamsUniformBuffers[i],
                    nullptr);
    vkFreeMemory(VulkanCore::getDevice(), gridParamsUniformBuffersMemory[i],
                 nullptr);
  }
}
...
void GridPlane::updateUniformBuffer(uint32_t currentImage,
  ...
  void* data;
  vkMapMemory(VulkanCore::getDevice(),
              gridPlaneUniformBuffersMemory[currentImage], 0, sizeof(ubo), 0,
              &data);
  memcpy(data, &ubo, sizeof(ubo));
  vkUnmapMemory(VulkanCore::getDevice(),
                gridPlaneUniformBuffersMemory[currentImage]);
}

void GridPlane::updateGridParamsBuffer(unsigned int currentImage, const glm::vec3& cameraWorldPos, float gridSize,
  ...
  void* data;
  vkMapMemory(VulkanCore::getDevice(),
              gridParamsUniformBuffersMemory[currentImage], 0, sizeof(ubo), 0,
              &data);
  memcpy(data, &ubo, sizeof(ubo));
  vkUnmapMemory(VulkanCore::getDevice(),
                gridParamsUniformBuffersMemory[currentImage]);
}
...
void GridPlane::createGridPlanePipeline(VkRenderPass renderPass) {
  ...
  if (vkCreatePipelineLayout(VulkanCore::getDevice(), &pipelineLayoutInfo,
                             nullptr, &gridPlanePipelineLayout) != VK_SUCCESS) {
  ...
  if (vkCreateGraphicsPipelines(VulkanCore::getDevice(), VK_NULL_HANDLE, 1,
                                &pipelineInfo, nullptr,
                                &gridPLanePipeline) != VK_SUCCESS) {
...
void GridPlane::createDescriptorSets() {
  ...
  if (vkCreateDescriptorPool(VulkanCore::getDevice(), &poolInfo, nullptr,
                             &gridPlaneDescriptorPool) != VK_SUCCESS) {
  ...
  std::array<VkDescriptorSetLayoutBinding, 2> bindings = {
      samplerLayoutBinding, gridParamsLayoutBinding};
  ...
  if (vkAllocateDescriptorSets(VulkanCore::getDevice(), &allocInfo,
                               gridPlaneDescriptorSets.data()) != VK_SUCCESS) {
...
void GridPlane::createGridPlaneUniformBuffers() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    Utils::createBuffer(gridBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
