...
void ViewPort::init(VulkanCore* core) {
	...
	if (vkCreateCommandPool(engineCore->getDevice(), &commandPoolCreateInfo, nullptr, &m_ViewportCommandPool) != VK_SUCCESS) {
	...
	if (vkAllocateCommandBuffers(engineCore->getDevice(), &allocInfo, m_ViewportCommandBuffers.data()) != VK_SUCCESS)
	{
...
void ViewPort::cleanupFramebuffers() {
	...
	for (auto imageView : m_ViewportImageViews) {
		vkDestroyImageView(engineCore->getDevice(), imageView, nullptr);
	}
...
void ViewPort::cleanup() {
	...
	vkDestroyCommandPool(engineCore->getDevice(), m_ViewportCommandPool, nullptr);

	vkDestroyRenderPass(engineCore->getDevice(), m_ViewportRenderPass, nullptr);
}
...
void ViewPort::createViewportImage() {
	...
	for (uint32_t i = 0; i < engineCore->getSwapChainImageViews().size(); i++)
	{
		...
		// Create the image
		// VkImage dstImage;
		vkCreateImage(engineCore->getDevice(), &imageCreateCI, nullptr, &m_ViewportImages[i]);
		...
		vkBindImageMemory(engineCore->getDevice(), m_ViewportImages[i], m_DstImageMemory[i], 0);
		...
		vkAllocateCommandBuffers(engineCore->getDevice(), &allocInfo, &copyCmd);
		...
		vkQueueSubmit(engineCore->getGraphicsQueue(), 1, &submitInfo, VK_NULL_HANDLE);
		vkQueueWaitIdle(engineCore->getGraphicsQueue());

		vkFreeCommandBuffers(engineCore->getDevice(), m_ViewportCommandPool, 1, &copyCmd);
	}
}
...
void ViewPort::createViewportImageViews() {
	...
	{
		m_ViewportImageViews[i] = engineCore->createImageView(m_ViewportImages[i], VK_FORMAT_B8G8R8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}
}
...
void ViewPort::createViewportRenderPass() {
	...
	if (vkCreateRenderPass(engineCore->getDevice(), &renderPassInfo, nullptr, &m_ViewportRenderPass) != VK_SUCCESS) {
...
void ViewPort::createViewportFramebuffers() {
	...
	for (size_t i = 0; i < m_ViewportImageViews.size(); i++)
	{
		...
		if (vkCreateFramebuffer(engineCore->getDevice(), &framebufferInfo, nullptr, &m_ViewportFramebuffers[i]) != VK_SUCCESS)
		{
...
void ViewPort::recordViewportCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
	...
	if (vkBeginCommandBuffer(m_ViewportCommandBuffers[engineCore->getCurrentFrame()], &beginInfo) != VK_SUCCESS)
	{
	...
	vkCmdBeginRenderPass(m_ViewportCommandBuffers[engineCore->getCurrentFrame()], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

	vkCmdBindPipeline(m_ViewportCommandBuffers[engineCore->getCurrentFrame()], VK_PIPELINE_BIND_POINT_GRAPHICS, engineCore->getPipeline());
	...
	vkCmdBindIndexBuffer(commandBuffer, engineCore->getIndexBuffer(), 0, VK_INDEX_TYPE_UINT32);

	vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, engineCore->getPipelineLayout(), 0, 1, &engineCore->getDescriptorSets()[engineCore->getCurrentFrame()], 0, nullptr);

	vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);

	vkCmdEndRenderPass(commandBuffer);

	if (vkEndCommandBuffer(m_ViewportCommandBuffers[engineCore->getCurrentFrame()]) != VK_SUCCESS)
	{
