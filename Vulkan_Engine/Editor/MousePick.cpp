...
void MousePick::init(VulkanCore* core) {
  ...
  allocInfo.commandBufferCount =
      static_cast<uint32_t>(mousePickCommandBuffers.size());

  if (vkAllocateCommandBuffers(VulkanCore::getDevice(), &allocInfo,
                               mousePickCommandBuffers.data()) != VK_SUCCESS) {
    throw std::runtime_error("failed to allocate command buffers!");
  }

  MeshManager::loadDefaults(VulkanCore::getCommandPool(),
...
void MousePick::createMousePickImage() {
  ...
  for (uint32_t i = 0; i < engineCore->getSwapChainImageViews().size(); i++) {
    ...
    imageCreateCI.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
    ...
    vkGetImageMemoryRequirements(VulkanCore::getDevice(), mousePickImages[i],
                                 &memRequirements);
    ...
    if (vkAllocateMemory(VulkanCore::getDevice(), &memAllocInfo, nullptr,
                         &mousePickImageMemory[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to allocate image memory!");
    }
    vkBindImageMemory(VulkanCore::getDevice(), mousePickImages[i],
                      mousePickImageMemory[i], 0);
    ...
    vkFreeCommandBuffers(VulkanCore::getDevice(), VulkanCore::getCommandPool(),
                         1, &copyCmd);
  }
}
...
void MousePick::createMousePickImageViews() {
  ...
  for (uint32_t i = 0; i < mousePickImages.size(); i++) {
    mousePickImageViews[i] = Utils::createImageView(mousePickImages[i], VK_FORMAT_B8G8R8A8_UNORM,
...
void MousePick::createMousePickRenderPass() {
  ...
  if (vkCreateRenderPass(VulkanCore::getDevice(), &renderPassInfo, nullptr,
                         &mousePickRenderPass) != VK_SUCCESS) {
...
void MousePick::createMousePickFramebuffers() {
  ...
  for (size_t i = 0; i < mousePickImageViews.size(); i++) {
    ...
    if (vkCreateFramebuffer(VulkanCore::getDevice(), &framebufferInfo, nullptr,
                            &mousePickFramebuffers[i]) != VK_SUCCESS) {
...
uint32_t MousePick::getEntityIDAt(int x, int y) {
  ...
  vkCmdCopyImageToBuffer(cmdBuffer, srcImage,
