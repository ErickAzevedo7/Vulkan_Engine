...
void Utils::createShaderModule(const std::vector<char>& code, VkShaderModule& shaderModule) {
	...
	if (vkCreateShaderModule(VulkanCore::getDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
...
void Utils::createImage(uint32_t width,
	...
	if (vkAllocateMemory(VulkanCore::getDevice(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
...
void Utils::transitionImageLayout(VkImage image,
	...
	if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
		...
		// Used when transitioning a freshly-created image directly to shader-read
		// (e.g. a cleared image that won't go through a transfer).
		barrier.srcAccessMask = 0;
		barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
		...
		// Used to prepare a cubemap image for use as a colour attachment
		// (IBL offline render passes).
		barrier.srcAccessMask = 0;
		...
		destinationStage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
			   ...
			   newLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
		// Used after an IBL render pass: make the colour attachment readable
