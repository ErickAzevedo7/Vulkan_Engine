...
void Outline::init(VulkanCore* core,
  ...
  allocInfo.commandBufferCount =
      static_cast<uint32_t>(outlineCommandBuffers.size());

  if (vkAllocateCommandBuffers(VulkanCore::getDevice(), &allocInfo,
                               outlineCommandBuffers.data()) != VK_SUCCESS) {
...
void Outline::recordOutlineCommandBuffer(VkCommandBuffer commandBuffer,
                                         uint32_t imageIndex) {
...
void Outline::createOutlineFramebuffers() {
  ...
  for (size_t i = 0; i < outlineColorImageViews.size(); i++) {
    ...
    if (vkCreateFramebuffer(VulkanCore::getDevice(), &framebufferInfo, nullptr,
                            &outlineFramebuffers[i]) != VK_SUCCESS) {
...
void Outline::createGraphicsPipeline() {
  ...
  if (vkCreatePipelineLayout(VulkanCore::getDevice(), &pipelineLayoutInfo,
                             nullptr, &outlinePipelineLayout) != VK_SUCCESS) {
...
void Outline::createOutlineDescriptorSets() {
  ...
  if (vkCreateDescriptorSetLayout(VulkanCore::getDevice(), &layoutInfo, nullptr,
                                  &outlineDescriptorSetLayout) != VK_SUCCESS) {
  ...
  if (vkAllocateDescriptorSets(VulkanCore::getDevice(), &allocInfo,
                               outlineDescriptorSets.data()) != VK_SUCCESS) {
...
void Outline::createDescriptorPool() {
  ...
  if (vkCreateDescriptorPool(VulkanCore::getDevice(), &poolInfo, nullptr,
                             &outlineDescriptorPool) != VK_SUCCESS) {
